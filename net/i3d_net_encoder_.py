...
class encoder(torch.nn.Module):
	...
	#shape from(3,384,56,56)-->(64,128,28,28) 
	...
	#layer2:conv-bn-relu
	#shape from(64,128,28,28)-->(64,128,28,28)
    ...
    #layer3:conv-bn-relu
	#shape from(64,128,28,28)-->(192,32,28,28)
        self.conv3d_3_1_4x3x3 = torch.nn.Conv3d(64, out_channels, (4,3,3), stride=(1,1,1),padding=(0,1,1))
    ...
    def forward(self,x):
        ...
        out = self.bn1(out)
        out = self.relu1(out,inplace=True)
        out = self.maxpool3d_1(out)
        out = self.conv3d_1x3x3(out)
        out = self.bn2(out)
        out = self.relu2(out,inplace=True)
        out = self.conv3d_3_1_4x3x3(out)
        out = self.bn3_1(out)
        out = self.relu3_1(out,inplace=True)
        #out = self.conv3d_3_2_2x3x3(out)
        #out = self.bn3_2(out)
        #out = self.relu3_2(out,inplace=True)
        return out
class I3D_with_encoder(torch.nn.Module):
    def __init__(self, num_classes=400, modality='rgb', dropout_prob=0.5, name='inception'):
